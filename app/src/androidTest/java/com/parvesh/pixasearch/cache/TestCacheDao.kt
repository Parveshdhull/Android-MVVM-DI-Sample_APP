...
class TestCacheDao {
    ...
    fun setup() {
        ...
        post1 = PostEntity("key1", 123, "thumbnail", "Bob",
            "Tag1, Tag2", "largeImage", 12, 23, 34, "test", 1)
        post2 = PostEntity("key2", 123, "thumbnail", "Bob",
            "Tag1, Tag2", "largeImage", 12, 23, 34, "test", 1)
        list = ArrayList()
    }
    ...
    fun testMultiplePostsInsertionWithDifferentKeys() = runBlockingTest{
        ...
        dao.getPosts("test").test().assertValue { posts -> posts.size == 2 && posts[0] == post1 && posts[1] == post2 }
    }
    ...
    fun testSearchEntityInsertion() = runBlockingTest{
        var searchEntity = SearchEntity("test", 100)
